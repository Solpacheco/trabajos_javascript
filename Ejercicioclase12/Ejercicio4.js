...
let notas = []; // Array vacío para guardar las notas
let cantidadEstudiantes = parseInt(prompt("¿Cuántas notas querés ingresar?"));

for (let i = 0; i < cantidadEstudiantes; i++) {
    let nota = parseFloat(prompt("Ingresa la nota del estudiante " + (i + 1) + ":"));
    notas[i] = nota; // Guardamos la nota en la posición i del array
}
